...
// Load API key from environment variables (safer!)
...
const Index = () => {
  ...
  // NOTE: We have removed the client-side getSevereWeatherAlert function.
  // All alerts are now handled by the AlertSystem component,
  // which reads from the database populated by our new server-side function.
  ...
  const fetchWeatherData = async (city: string = '') => {
    ...
    if (!API_KEY) {
      setError('OpenWeatherMap API key is not configured. Please set VITE_OPENWEATHER_API_KEY in your .env file.');
      setLoading(false);
      return;
    }
  ...
  // This will now be populated by the AlertSystem component reading from Supabase
  // const severeWeatherAlert = weatherData ? getSevereWeatherAlert(weatherData) : null; 
      ...
      <div className="min-h-screen relative overflow-hidden">
        ...
        <div className="relative z-10 min-h-screen bg-black/20 backdrop-blur-sm">
          <div className="container mx-auto px-4 py-6 max-w-6xl">
            ...
            {/* This section is now redundant because the AlertSystem component
              will show all active alerts from the database.
            */}
            {/* {severeWeatherAlert && (
              <Alert className="mb-6 bg-orange-500/20 border-orange-500/50 text-white">
                <AlertTriangle className="h-4 w-4" />
            ...
            {weatherData && (
              <>
                ...
                {viewMode === 'aviation' && (
                  <div className="mb-8">
                    {/* Aviation view now includes the AlertSystem automatically */}
                    <AviationWeatherDashboard />
                  </div>
                )}
